...
class Slider extends React.Component {
	...
	nextOne(){
		(this.state.active < this.state.max - 1) ? this.setState({active: this.state.active + 1}) : this.setState({active: 0}); 
	}

	prevOne(){
		(this.state.active > 0) ? this.setState({active: this.state.active - 1}) : this.setState({active: this.state.max - 1}); 
	}
	...
	renderSlides(){
		...
		return this.state.slides.map((item, index) => (
			<div 
				...
				key={index} style={{backgroundImage: item.eachSlide, width: transition}}>
			</div>
		))
	}
	...
	renderPlayStop(){
		...
		if (this.state.autoplay){
			playStop = <svg height="24" viewBox="0 0 24 24" width="24" fill="#ffffff">
